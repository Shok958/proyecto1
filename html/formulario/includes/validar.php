<?php 
	...
	function validarNombre($nombre){
		...
		if (strlen($nombre)<3){ 
      	...
      	if (! preg_match("/^[a-zA-ZñÑáéíóúÁÉÍÓÚüÜ]+$/",$nombre)) {
	...
	function validarApellido($apellido){
		...
		if (empty($apellido)) {
			$_SESSION['errores']['apellido']= "campo nombre vacio";
		}

		if (strlen($apellido)<3){ 
      		$_SESSION['errores']['apellido']= "campo nombre demasiado corto";
      	}
      	if (! preg_match("/^[a-zA-ZñÑáéíóúÁÉÍÓÚüÜ]+$/",$apellido)) {
			$_SESSION['errores']['apellido']= "caracteres de nombre no validos";
		}
	}
